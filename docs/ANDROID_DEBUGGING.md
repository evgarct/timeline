...
4. **Посторонние push-уведомления или сам пользователь телефона перехватывают тап во время автотеста:**
   - Любое heads-up уведомление от другого приложения (Telegram, Gmail и т.п.) может всплыть поверх Form ровно в момент `adb shell input tap` и перехватить нажатие. Хуже: это физический телефон пользователя — он может в любой момент сам взять его и переключиться на другое приложение (наблюдалось: TikTok) независимо от автотеста, и это не разовая помеха, а устойчивое состояние на несколько минут.
   - **Диагностика:** сразу после неожиданного результата выполнить `adb shell dumpsys activity activities | grep topResumedActivity` — если там `com.evgarct.form/.MainActivity`, тап просто попал в баннер поверх Form (разовая помеха); если там другой пакет (например `com.zhiliaoapp.musically`), значит пользователь сейчас активно пользуется телефоном сам.
   - **Решение при разовой помехе (баннер):** `adb shell input keyevent 4` (Back), чтобы закрыть баннер, и повторить тап.
   - **Решение при устойчивом переключении (пользователь пользуется телефоном):** НЕ повторять тапы вслепую, надеясь попасть обратно в Form — случайный тап в чужом активном приложении (лайк, подписка, отправка сообщения) необратим и не относится к автотесту. Повторно проверить `topResumedActivity` (read-only, безопасно); если сторонний пакет остаётся, остановить автоматизацию, явно сообщить пользователю, что телефон сейчас у него, и попросить освободить устройство или подтвердить, что можно продолжать — не выдавать это за "не удалось воспроизвести баг".
