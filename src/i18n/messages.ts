import type { Locale } from "./config";

const messages = {
  ru: {
    brand: "Fitness Timeline",
    hero: "Вся твоя история формы в одном месте",
    heroBody: "Фотографии, тренировки, замеры и тесты тела — в хронологии, которую ты создаешь каждый день.",
    start: "Начать",
    signIn: "Войти",
    email: "Email",
    continue: "Продолжить",
    otp: "Код из письма",
    sendCode: "Получить код",
    verifyCode: "Подтвердить код",
    codeSent: "Код отправлен на email",
    authError: "Не удалось выполнить вход",
    today: "Сегодня",
    tasks: "Действия сегодня",
    current: "Текущее состояние",
    recent: "Последние события",
    timeline: "Timeline",
    workout: "Тренировка",
    measurements: "Замеры тела",
    progressPhoto: "Новая фотосессия",
    inbody: "InBody",
    settings: "Настройки",
    edit: "Редактировать",
    delete: "Удалить событие",
    note: "Заметка",
    photos: "Выбрать из Photos",
    camera: "Снять камерой",
    addEvent: "Добавить событие",
    stale: "Нет свежих данных за 30 дней",
    completed: "Выполнено",
    everySunday: "Каждое воскресенье",
    weekly: "Раз в неделю",
    bodyHistory: "История тела",
    mcp: "MCP developer preview",
    token: "Персональный токен",
    createToken: "Создать токен",
    profile: "Профиль",
    schedule: "Периодичность задач",
    uploadReport: "Загрузить отчет",
    reviewReport: "Проверить распознанные данные",
    save: "Сохранить",
    cancel: "Отмена",
    preparingUpload: "Подготавливаем фотографии...",
    convertingUpload: "Конвертируем изображение...",
    uploading: "Загружаем...",
    uploadFailed: "Не удалось загрузить файл. Попробуйте еще раз.",
    unsupportedFormat: "Поддерживаются HEIC, HEIF, JPEG и PNG.",
    fileTooLarge: "Размер файла не должен превышать 25 МБ.",
    tooManyPhotos: "Можно загрузить не больше 12 фотографий.",
    openGallery: "Открыть галерею",
    photoCount: "Фотографий: {count}",
    openReport: "Открыть оригинал отчета",
    deleteConfirmTitle: "Удалить событие?",
    deleteConfirmDescription: "Файлы будут сразу удалены из приватного хранилища. Это действие нельзя отменить.",
    deleteFailed: "Не удалось удалить событие.",
    home: "Лендинг",
    openProduct: "Открыть продукт",
    storage: "Хранилище",
    storageUsed: "Использовано {used} из {limit} МБ",
    storageUnlimited: "Без лимита",
    storageQuotaExceeded: "Место закончилось. Сейчас доступно 250 МБ.",
    openMenu: "Открыть меню",
    openPhoto: "Открыть фото",
    swipePhoto: "Листать ракурсы",
    scrollTimeline: "Прокрутить к Timeline",
    morning: "Утро",
    beforeWorkout: "До тренировки",
    atHome: "Дом",
    bodyData: "Данные тела",
    hideBodyData: "Скрыть данные тела",
    moreActions: "Еще действия",
    notCompleted: "Не выполнено",
    addPhoto: "Добавить фотографию",
    markWorkout: "Отметить тренировку",
    viewDetails: "Посмотреть детали",
    weight: "Вес",
    waist: "Талия",
    bodyFat: "Процент жира",
    photoNote: "Комментарий",
    noPhotoTitle: "Добавь первую фотосессию",
    noPhotoBody: "Сегодня станет точкой отсчета для истории формы."
  },
  en: {
    brand: "Fitness Timeline",
    hero: "Your complete body story in one place",
    heroBody: "Photos, workouts, measurements and body tests — in the chronology you create every day.",
    start: "Get started",
    signIn: "Sign in",
    email: "Email",
    continue: "Continue",
    otp: "Email code",
    sendCode: "Send code",
    verifyCode: "Verify code",
    codeSent: "Code sent to your email",
    authError: "Could not sign in",
    today: "Today",
    tasks: "Today’s actions",
    current: "Current state",
    recent: "Recent events",
    timeline: "Timeline",
    workout: "Workout",
    measurements: "Body measurements",
    progressPhoto: "New photo session",
    inbody: "InBody",
    settings: "Settings",
    edit: "Edit",
    delete: "Delete event",
    note: "Note",
    photos: "Choose from Photos",
    camera: "Take a photo",
    addEvent: "Add event",
    stale: "No fresh data in the last 30 days",
    completed: "Completed",
    everySunday: "Every Sunday",
    weekly: "Once a week",
    bodyHistory: "Body history",
    mcp: "MCP developer preview",
    token: "Personal token",
    createToken: "Create token",
    profile: "Profile",
    schedule: "Task schedule",
    uploadReport: "Upload report",
    reviewReport: "Review extracted data",
    save: "Save",
    cancel: "Cancel",
    preparingUpload: "Preparing photos...",
    convertingUpload: "Converting image...",
    uploading: "Uploading...",
    uploadFailed: "The upload failed. Try again.",
    unsupportedFormat: "HEIC, HEIF, JPEG and PNG are supported.",
    fileTooLarge: "Files must be 25 MB or smaller.",
    tooManyPhotos: "You can upload up to 12 photos.",
    openGallery: "Open gallery",
    photoCount: "Photos: {count}",
    openReport: "Open original report",
    deleteConfirmTitle: "Delete this event?",
    deleteConfirmDescription: "Its files will be removed from private storage immediately. This cannot be undone.",
    deleteFailed: "Could not delete the event.",
    home: "Landing",
    openProduct: "Open product",
    storage: "Storage",
    storageUsed: "Used {used} of {limit} MB",
    storageUnlimited: "Unlimited",
    storageQuotaExceeded: "Storage is full. The current limit is 250 MB.",
    openMenu: "Open menu",
    openPhoto: "Open photo",
    swipePhoto: "Swipe angles",
    scrollTimeline: "Scroll to Timeline",
    morning: "Morning",
    beforeWorkout: "Before workout",
    atHome: "Home",
    bodyData: "Body data",
    hideBodyData: "Hide body data",
    moreActions: "More actions",
    notCompleted: "Not completed",
    addPhoto: "Add photo",
    markWorkout: "Mark workout",
    viewDetails: "View details",
    weight: "Weight",
    waist: "Waist",
    bodyFat: "Body fat",
    photoNote: "Note",
    noPhotoTitle: "Add the first photo session",
    noPhotoBody: "Today becomes the starting point for your body history."
  },
  cs: {
    brand: "Fitness Timeline",
    hero: "Celá historie tvé formy na jednom místě",
    heroBody: "Fotky, tréninky, měření a tělesné testy — v chronologii, kterou vytváříš každý den.",
    start: "Začít",
    signIn: "Přihlásit se",
    email: "E-mail",
    continue: "Pokračovat",
    otp: "Kód z e-mailu",
    sendCode: "Poslat kód",
    verifyCode: "Ověřit kód",
    codeSent: "Kód byl odeslán na e-mail",
    authError: "Přihlášení se nezdařilo",
    today: "Dnes",
    tasks: "Dnešní akce",
    current: "Aktuální stav",
    recent: "Poslední události",
    timeline: "Timeline",
    workout: "Trénink",
    measurements: "Měření těla",
    progressPhoto: "Nové focení",
    inbody: "InBody",
    settings: "Nastavení",
    edit: "Upravit",
    delete: "Smazat událost",
    note: "Poznámka",
    photos: "Vybrat z Fotek",
    camera: "Vyfotit",
    addEvent: "Přidat událost",
    stale: "Žádná aktuální data za 30 dní",
    completed: "Hotovo",
    everySunday: "Každou neděli",
    weekly: "Jednou týdně",
    bodyHistory: "Historie těla",
    mcp: "MCP developer preview",
    token: "Osobní token",
    createToken: "Vytvořit token",
    profile: "Profil",
    schedule: "Plán úkolů",
    uploadReport: "Nahrát report",
    reviewReport: "Zkontrolovat rozpoznaná data",
    save: "Uložit",
    cancel: "Zrušit",
    preparingUpload: "Připravujeme fotky...",
    convertingUpload: "Převádíme obrázek...",
    uploading: "Nahráváme...",
    uploadFailed: "Soubor se nepodařilo nahrát. Zkuste to znovu.",
    unsupportedFormat: "Podporujeme HEIC, HEIF, JPEG a PNG.",
    fileTooLarge: "Soubor může mít maximálně 25 MB.",
    tooManyPhotos: "Můžete nahrát maximálně 12 fotografií.",
    openGallery: "Otevřít galerii",
    photoCount: "Fotografií: {count}",
    openReport: "Otevřít původní report",
    deleteConfirmTitle: "Smazat událost?",
    deleteConfirmDescription: "Soubory budou ihned odstraněny ze soukromého úložiště. Tuto akci nelze vrátit.",
    deleteFailed: "Událost se nepodařilo smazat.",
    home: "Landing",
    openProduct: "Otevřít produkt",
    storage: "Úložiště",
    storageUsed: "Využito {used} z {limit} MB",
    storageUnlimited: "Bez limitu",
    storageQuotaExceeded: "Úložiště je plné. Aktuální limit je 250 MB.",
    openMenu: "Otevřít menu",
    openPhoto: "Otevřít fotku",
    swipePhoto: "Listovat úhly",
    scrollTimeline: "Přejít na Timeline",
    morning: "Ráno",
    beforeWorkout: "Před tréninkem",
    atHome: "Domov",
    bodyData: "Tělesná data",
    hideBodyData: "Skrýt tělesná data",
    moreActions: "Další akce",
    notCompleted: "Nehotovo",
    addPhoto: "Přidat fotku",
    markWorkout: "Označit trénink",
    viewDetails: "Zobrazit detaily",
    weight: "Váha",
    waist: "Pas",
    bodyFat: "Tělesný tuk",
    photoNote: "Poznámka",
    noPhotoTitle: "Přidej první focení",
    noPhotoBody: "Dnešek bude výchozím bodem historie tvé formy."
  }
} as const;

export type Copy = (typeof messages)["ru"];

export function getMessages(locale: Locale): Copy {
  return messages[locale] as Copy;
}
