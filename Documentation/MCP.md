...
Инструменты возвращают человекочитаемый текст плюс `structuredContent` с данными.
Интерактивной UI-карточки (MCP Apps / `ui://…`) больше нет — она не отображалась в
Claude и занимала место пустым блоком. Дневные итоги БЖУ доступны текстом:
`get_nutrient_details` с `date` + `timezone`, а также `get_today` при передаче
`timezone`.
