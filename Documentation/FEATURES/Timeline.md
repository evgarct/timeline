...
Progress-photo sessions render as large inline visual records, using the signed thumbnail in the scrolling archive and loading full media only in the existing gallery. A session opens the gallery on tap; private URLs remain transient display data.
...
Each measurement record compares itself with the immediately preceding measurement event. Increased, decreased, and unchanged values use neutral directional glyphs and numeric deltas; the interface does not assign success/failure colors or imply whether a change is good.
